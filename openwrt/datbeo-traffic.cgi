...
# Fallback for uhttpd/BusyBox clients where custom HTTP headers are not exposed
# to the CGI environment. The generated API key is hexadecimal, so no URL
# decoding is required here.
if [ -z "$REQ" ] && [ -n "$QUERY_STRING" ]; then
    REQ="$(printf '%s' "$QUERY_STRING" | sed -n 's/^key=\([^&]*\).*$/\1/p')"
fi
